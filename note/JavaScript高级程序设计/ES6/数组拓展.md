...
fill()方法还可以接受第二个和第三个参数，用于制定填充的其实位置和结束位置。
```js
  ['a', 'b', 'c'].fill(7, 1, 2)
  // ['a', 7, 'c']
```
上面代码表示，fill方法从1号位置开始，向原数组填充7，到2号位置结束。
...
ES6提供三个新方法————entries(), keys()和values()一一用于遍历数组。它们都返回一个遍历器对象，可以用for...of循环进行遍历，
唯一的区别是keys()是对键名的遍历、values()是对键值的遍历，entries()是对键值的遍历。
```js
...
Array.prototype.includes方法返回一个布尔值，表示某个数组是否包含给定的值，与字符串的includes方法类似。ES2016 引入了该方
法。
...
注意，空位不是undefined，某一个位置的值等于undefined，依然是有值得。空位是没有任何值，in运算符可以说明这一点。
```js
